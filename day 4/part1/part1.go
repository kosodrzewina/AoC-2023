...
func getPoints(card card) int {
	...
	return int(math.Pow(2, float64(playerWinningNumberCount-1)))
}
...
func getCards(filePath string) ([]card, error) {
	...
	for scanner.Scan() {
		...
		numberSets := strings.Split(strings.Split(scanner.Text(), ":")[1], "|")
		...
		cards = append(cards, card{winningNumbers: winningNumbers, numbers: numbers})
	}
